...
const Saved = ({ user }) => {
  ...
  const sortByEasy = () => {
    const sortedPosts = posts.filter((post) => post.difficulty === "EASY");
  ...
  const sortByHard = () => {
    const sortedPosts = posts.filter((post) => post.difficulty === "HARD");

    return sortedPosts;
  };

  const sortByMichelin = () => {
    const sortedPosts = posts.filter((post) => post.difficulty === "MICHELIN");
  ...
  return (
    <div className="mx-5 my-10">
      ...
      <section className="flex flex-col items-center justify-center">
        <h1 className="text-5xl font-thin mb-5">SAVED POSTS</h1>
        <section className="flex justify-center">
          <input
            ...
            placeholder="Search for your saved recipes..."
